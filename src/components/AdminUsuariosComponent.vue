<template>
  <div class="space-y-6">
    <!-- 1. FORMULARIO PARA AGREGAR USUARIO -->
    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
      ...
      >
        ...
        <div>
          <button
            type="submit"
            class="w-full bg-brand hover:bg-brand-dark text-white text-sm font-semibold py-2 px-4 rounded-xl transition-colors shadow-sm active:scale-[0.99]"
          >
            Agregar Usuario
          </button>
        </div>
    ...
    <!-- 2. SECCIÓN DE PERSONAL (Contenedor adaptable) -->
    <div class="space-y-4">
      <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider text-gray-500 px-2">
        Personal de la Plataforma
      </h3>

      <!-- 🖥️ EN LARGOS: SE MUESTRA LA TABLA TRADICIONAL -->
      <div
          ...
          <tbody class="divide-y divide-gray-100 text-sm">
            ...
            >
              <td class="py-4 px-6">
                ...
                <div class="text-xs text-gray-400">{{ user.correo }}</div>
              </td>
              <td class="py-4 px-6">
                <span
                  :class="badgeRol(user.rol)"
                  class="px-2 py-0.5 text-[11px] font-bold rounded-full uppercase"
                >
                  {{ user.rol }}
                </span>
              </td>
                ...
                <button
                  ...
                  :class="
                    user.activo
                      ...
                      : 'text-brand hover:bg-purple-50 border border-brand/20'
                  "
      ...
      <!-- 📱 EN MÓVILES: SE OCULTA LA TABLA Y SE MUESTRAN CARDS ESTÉTICAS -->
      <div class="block md:hidden grid grid-cols-1 gap-4">
        ...
        >
          <!-- Encabezado de la Card: Nombre y Botón de Acción -->
          <div class="flex justify-between items-start gap-2">
            ...
            <button
              @click="toggleEstado(user)"
              :class="user.activo ? 'text-red-600 bg-red-50' : 'text-brand bg-purple-50'"
              class="px-2.5 py-1 text-xs font-bold rounded-lg transition-all active:scale-95"
            >
              {{ user.activo ? 'Apagar' : 'Prender' }}
            </button>
          </div>

          <!-- Footer de la Card: Badges de Rol y Estado -->
...
const listaUsuarios = ref([
  ...
  {
    id: 3,
    nombre: 'Lic. Fernando G.',
    correo: 'fernando.cap2@gob.mx',
    rol: 'capturista',
    activo: true,
  },
  {
    id: 4,
    nombre: 'Dr. Ramiro Pozos',
    correo: 'ramiro.consultor@gob.mx',
    rol: 'consultor',
    activo: true,
  },
  {
    id: 5,
    nombre: 'Mtra. Elena Ruiz',
    correo: 'elena.baja@gob.mx',
    rol: 'consultor',
    activo: false,
  },
])
...
const badgeRol = (rol) => {
  if (rol === 'super') return 'bg-purple-100 text-purple-700'
  if (rol === 'capturista') return 'bg-blue-100 text-blue-700'
