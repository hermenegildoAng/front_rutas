  ...
  <div class="bg-white/95 backdrop-blur-md rounded-3xl shadow-2xl border border-white/40 p-4 space-y-3">
    ...
    >
      ...
      <template v-if="!archivoSeleccionado && !archivoCargando">
        ...
        <div>
          ...
          <p class="text-[10px] text-gray-400 mt-0.5">Soporta GeoJSON, GPX, KML</p>
        </div>
      </template>

      <template v-else-if="archivoCargando">
        <div class="w-6 h-6 border-2 border-purple-200 border-t-brand rounded-full animate-spin"/>
        <p class="text-xs font-medium text-gray-500">Procesando geometría...</p>
      </template>
      ...
      <template v-else>
        ...
        <div class="max-w-[180px] truncate">
          ...
          <p class="text-[10px]" :class="archivoValido ? 'text-emerald-600' : 'text-rose-500'">
            {{ archivoValido ? 'Archivo listo' : 'Formato no soportado' }}
          </p>
        </div>
      </template>
    </div>
    ...
    <div v-if="archivoSeleccionado && archivoValido && !archivoCargando" class="flex gap-2">
      <button
        ...
        class="flex-1 px-3 py-1.5 border border-gray-200 hover:bg-gray-50 text-gray-600 rounded-xl text-xs font-semibold transition-all"
      >
        Cancelar
      </button>
      <button
        @click.stop="procesarArchivo"
        class="flex-1 px-3 py-1.5 bg-brand hover:bg-purple-700 text-white rounded-xl text-xs font-semibold shadow-md shadow-purple-200 transition-all"
      >
        Interpolar
      </button>
    </div>
    ...
    <div
      ...
      :class="[
        'p-2.5 rounded-xl text-[11px] font-medium flex items-center gap-2 border',
...
const {
  ...
  // Cuando el composable tiene los puntos listos, se los manda al mapa principal
  emit('puntos-cargados', puntos)
})

const validarExtension = (nombre) => {
  const ext = '.' + nombre.split('.').pop().toLowerCase()
  return ['.geojson', '.json', '.gpx', '.kml'].includes(ext)
}
