<template>
  <div class="relative w-full h-[calc(100vh-90px)] overflow-hidden rounded-2xl">
    ...
    <Transition name="slide-left">
      <div
      v-show="formularioVisible"
      class="absolute top-4 left-4 z-30 w-[420px] max-w-[95vw] max-h-[calc(100vh-122px)] bg-white/95 backdrop-blur-md rounded-3xl shadow-2xl border border-white/40 overflow-hidden flex flex-col"
    >
      <!-- HEADER -->
          ...
          <div class="space-y-4 pt-2">
            <template v-for="(parada, index) in form.paradas" :key="index">
              <div class="relative border border-gray-200 rounded-2xl p-4 bg-gray-50 space-y-4 pt-10">
                ...
                <div>
                  <label class="block text-xs font-semibold text-gray-500 mb-1">Tiempo Espera (min)</label>
                  <input type="number" v-model="parada.tiempo_espera" class="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-white text-sm outline-none focus:ring-2 focus:ring-purple-500"/>
                </div>
              </div>
              <div v-if="index < form.paradas.length - 1" class="flex justify-center">
                <div class="bg-white border border-dashed border-brand rounded-xl px-4 py-3 w-full">
                  <label class="block text-xs font-semibold text-gray-500 mb-1">Tiempo entre parada {{ index + 1 }} y {{ index + 2 }}</label>
                  <input type="number" v-model="form.tiemposEntreParadas[index]" class="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-white text-sm outline-none focus:ring-2 focus:ring-purple-500" placeholder="Minutos"/>
                </div>
              </div>
            </template>
          </div>
    ...
    <!-- ========================= PANEL DERECHO ========================= -->
    <div class="absolute top-4 right-[3.5rem] mr-8 z-20 w-72 flex flex-col gap-2">

      <!-- CARGADOR DE ARCHIVO — panel separado encima -->
      <div class="bg-white/95 backdrop-blur-md rounded-3xl shadow-2xl border border-white/40 p-4 space-y-3">
        ...
        <button
          ...
          class="w-full py-2.5 rounded-xl bg-brand text-white text-sm font-semibold hover:opacity-90 transition-opacity shadow-sm flex items-center justify-center gap-2"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-4 h-4">
...
const camposGenerales = [
  ...
  { key: 'tipo_ruta', label: 'Tipo Ruta', opciones: ['Autobús Urbano', 'Microbús', 'BRT'] },
]
...
const agregarParada = (lat = '', lng = '') => {
  ...
  form.value.paradas.push({ folio_parada: '', nombre_parada: `Parada ${orden}`, latitud: lat, longitud: lng, tiempo_espera: '', orden_parada: orden })
  if (form.value.paradas.length > 1) form.value.tiemposEntreParadas.push('')
...
const eliminarParada = (index) => {
  ...
  const it = index === form.value.paradas.length ? index - 1 : index
  if (form.value.tiemposEntreParadas.length > 0 && it >= 0) form.value.tiemposEntreParadas.splice(it, 1)
...
const procesarArchivo = () => {
  ...
  // Aquí conectas tu lógica de parseo: GeoJSON, GPX, KML…
  console.log('Procesando archivo:', archivoSeleccionado.value.name)
  alert(`Archivo "${archivoSeleccionado.value.name}" listo para procesar.`)
}
...
const handleGuardarRuta = () => {
  ...
  console.log(payload)
  alert('Ruta guardada (revisa consola)')
}
</script>
<style>
/* Formulario — desliza desde la izquierda */
...
/* Lista de puntos — colapsa verticalmente */
